...
$storePath = Join-Path $storeRoot "discord-bot.dpapi"
$entropy = [Text.Encoding]::UTF8.GetBytes("personal-context-hub/discord-bot/v1")
...
if ($Action -eq "write") {
  ...
  if ([string]::IsNullOrWhiteSpace($plaintext)) {
    throw "Refusing to store an empty Discord credential."
  }
  ...
  $temporaryPath = Join-Path $storeRoot ("discord-bot." + [Guid]::NewGuid() + ".tmp")
  try {
...
if (-not (Test-Path -LiteralPath $storePath -PathType Leaf)) {
  throw "The Windows DPAPI Discord credential does not exist. Store a Bot token again."
}
