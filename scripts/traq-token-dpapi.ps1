...
$storePath = Join-Path $storeRoot "traq-oauth.dpapi"
$entropy = [Text.Encoding]::UTF8.GetBytes("personal-context-hub/traq-oauth/v1")
...
if ($Action -eq "write") {
  ...
  if ([string]::IsNullOrWhiteSpace($plaintext)) {
    throw "Refusing to store an empty traQ credential."
  }
  ...
  $temporaryPath = Join-Path $storeRoot ("traq-oauth." + [Guid]::NewGuid() + ".tmp")
  try {
...
if (-not (Test-Path -LiteralPath $storePath -PathType Leaf)) {
  throw "The Windows DPAPI traQ credential does not exist. Re-run OAuth authentication."
}
